...
class GUI {
    public:
        int drawWindow(int x, int y, int width, int height, string title, WORD color = defColor, WORD borderColor = defColor) {
            ...
            WriteText(x, y, "Ъ");
            WriteText(x + width - 1, y, "ї");
            WriteText(x, y + height - 1, "А");
            WriteText(x + width - 1, y + height - 1, "Щ");

            for (int i = 1; i < width - 1; i++) {
                WriteText(x + i, y, "Д");
                WriteText(x + i, y + height - 1, "Д");
            }
            for (int i = 1; i < height - 1; i++) {
                WriteText(x, y + i, "і");
                WriteText(x + width - 1, y + i, "і");
            }

            WriteText(x + 1, y, "ґ");
            WriteText(x + 2 + title.size(), y, "Г");
        ...
        int drawButtonO(int x, int y, string caption, string key, WORD color = defColor, WORD borderColor = defColor) {
            ...
            WriteText(x + 4 + key.size() + caption.size(), y, "[ю]");
        ...
        int buttonOupd(int id) {
            ...
            SetColor(color);
            WriteText(_x + 4 + _key.size() + _caption.size(), _y, "[ю]");
    ...
    if (window == "sign_in") {
        ...
        while (window == "sign_in") {
            ...
            if (ch == ENTER) {
				...
				string nick = gui.getInputText(nickname);  // Эту переменную нужно сохранить, дальше пользователь будет под этим ником в чате писать
				if (nick != "") {
					if (pass != "") {
						if (pass == password) {
